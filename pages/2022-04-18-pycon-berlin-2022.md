...
covers two major groups of topics. The first one is a broad range of general software engineering topics. 
And the second one includes talks about doing data analysis and machine learning using Python's numerical stack. 
(Which is probably not a big surprise for people closely following the language's development and applications.) 
Therefore, the topics described here can be roughly categorized into two (overlapping) sets: 🐍 Python Insights and 
⚛️ Data Science️. I mark each summary with these emojis to make it a bit easier to navigate.
...
to be not that difficult! That's why I liked this talk so much. It takes a goal that looks quite intimidating at first
glance: changing the grammar of Python interpreter. And then the speaker shows that it is not that difficult to achieve.
...
Isn't that great? Also, the speaker points to two sources of information for people who want to know more about CPython 
internals and/or get involved into the language development process. (See [references](#refs).) This talk ignited my 
interest in hacking CPython to get more insights about its implementation.
